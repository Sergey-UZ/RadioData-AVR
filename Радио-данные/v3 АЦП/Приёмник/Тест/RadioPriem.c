...
/* 
   ...
   Флаг устанавливается в 1, если в момент завершения приёма 1-го пользовательского
...
void RadioPriem(void)
{
	...
	static enum Paket
	{
		...
		Reset    = 3, // Сброс радио-приёма
	} Paket = StartBit;
	/*
	   Устанавливается в 1 для синхронизации (в момент первого обнаружения PIN = 1)
	    при поиске стартового бита пакета
	*/ 
	static char FlagSinhronizaciya = 1;
	static unsigned char PoluBitStrobCount = 0; // Кол-во прошедшых стробирующих периудов
	// Уровень принятого сигнала в каждом полубите информационного бита
	static unsigned int PoluBit_A_Count = 0; // Уровень сигнала полубита A
	static unsigned int PoluBit_B_Count = 0; // Уровень сигнала полубита B
